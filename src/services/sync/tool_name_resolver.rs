use crate::model::{ParseError, Skill, build_auto_tool_name, validate_explicit_tool_name};
    ...
    for skill in skills.iter_mut() {
        ...
        skill.tool_name = match explicit {
            Some(explicit) => match validate_explicit_tool_name(&explicit) {
                ...
                None => {
                    eprintln!(
                        "Invalid tool_name in {}. Falling back to an auto-generated tool ID.",
                        skill.path
                    );
    ...
    for (tool_name, indices) in collisions {
        ...
        if strict {
            ...
            for index in indices {
                ...
                eprintln!(
                    "Duplicate tool_name '{}' detected in {}. Renamed to '{}'.",
                    tool_name, path, unique_name
                );
